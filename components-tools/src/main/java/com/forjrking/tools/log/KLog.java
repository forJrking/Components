package com.forjrking.tools.log;
...
public class KLog {
    ...
    public static TagHead processTagAndHead(String tag) {
    ...
    public static String processBody(final int type, final Object... contents) {
        String body = NULL;
        if (contents != null) {
    ...
    public static void print2File(final int type, final String tag, final String msg) {
        ...
        String time = format.substring(9);
        final String fullPath = CONFIG.getDir() + CONFIG.getFilePrefix() + "_" + date + ".txt";
    ...
    /**
     * DES: 文件名上带有20190320日期的时间戳 推算日期低于天数的文件删除
     * TIME: 2019/3/20 0020 上午 11:52
     */
    private static void deleteDueLogs(String filePath) {
        ...
        File[] files = parentFile.listFiles(new FilenameFilter() {
            ...
            public boolean accept(File dir, String name) {
                return name.matches("^" + CONFIG.getFilePrefix() + "_[0-9]{4}[0-9]{2}[0-9]{2}" + ".txt$");
            }
        });
        if (files == null || files.length <= 0) return;
        int length = CONFIG.getFilePrefix().length();
        final SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd", Locale.getDefault());
        try {
            String curDay = file.getName().substring(length + 1, file.getName().length() - 4);
            long dueMillis = sdf.parse(curDay).getTime() - CONFIG.getSaveDays() * 86400000L;
            for (final File aFile : files) {
                String name = aFile.getName();
                String logDay = name.substring(length + 1, name.length() - 4);
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = Cxt.get()
                    .getPackageManager()
                    .getPackageInfo(Cxt.get().getPackageName(), 0);
            if (pi != null) {
        ...
        final String head = "************* Log Head ****************" +
                "\nLog of Process     : " + CONFIG.getProcessName() +
    ...
    public static class Config {
        ...
        private boolean mLogSwitch = false;  // The switch of log.
        ...
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && Cxt.get().getExternalCacheDir() != null)
                mDefaultDir = Cxt.get().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                mDefaultDir = Cxt.get().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
            mDefaultDir = mDefaultDir + date + FILE_SEP;
        }
        ...
        private static String getCurrentProcessName() {
            ActivityManager am = (ActivityManager) Cxt.get().getSystemService(Context.ACTIVITY_SERVICE);
            if (am == null) return "";
    ...
    private static class LogFormatter {
        ...
        static com.google.gson.Gson getGson() {
            if (GSON == null) {
                GSON = new com.google.gson.GsonBuilder().setPrettyPrinting().serializeNulls().create();
            }
            return GSON;
        }
